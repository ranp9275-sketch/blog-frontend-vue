<template>
  <div>
    <h1 class="text-4xl font-bold mb-8">我的收藏</h1>

    <!-- 未登陆提示 -->
    <div v-if="!isAuthenticated" class="bg-blue-100 border border-blue-400 text-blue-700 px-6 py-4 rounded-lg mb-8">
      <p>请先 <router-link to="/login" class="underline font-semibold">登陆</router-link> 以查看收藏的文章</p>
    </div>

    <!-- 收藏列表 -->
    <div v-else>
      <div v-if="favorites.length > 0" class="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        <div
          v-for="article in favorites"
          :key="article.id"
          class="bg-white rounded-lg shadow-md hover:shadow-xl transition overflow-hidden cursor-pointer fade-in relative"
      ...
      <div v-else class="bg-gray-100 rounded-lg p-12 text-center">
        <p class="text-gray-600 text-lg">还没有收藏任何文章</p>
